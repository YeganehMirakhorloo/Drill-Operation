...
def train_drilling_models(data):
    ...
    print(f"ROP target shape: {y_rop.shape}")
    print(f"ROP statistics: min={y_rop.min():.2f}, max={y_rop.max():.2f}, mean={y_rop.mean():.2f}")
    ...
    rop_model = DrillingPredictor(
        hidden_neurons=23,  # Based on the paper's optimal 23 neurons
        learning_rate=0.001,
    ...
    print(f"ROP Model - RMSE: {rop_results['rmse']:.4f}, R²: {rop_results['r2']:.4f}, AARE: {rop_results['aare']:.2f}%")
    
    # Train Torque model
    print("\nTraining Torque Model...")
    y_torque = data['Surface_Torque'].values
    print(f"Torque target shape: {y_torque.shape}")
    print(f"Torque statistics: min={y_torque.min():.2f}, max={y_torque.max():.2f}, mean={y_torque.mean():.2f}")
    ...
    torque_model = DrillingPredictor(
        hidden_neurons=27,  # Based on the paper's optimal 27 neurons
        learning_rate=0.001,
        batch_size=8,
        epochs=300
    )
    ...
    print(f"Torque Model - RMSE: {torque_results['rmse']:.4f}, R²: {torque_results['r2']:.4f}, AARE: {torque_results['aare']:.2f}%")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Train models
        rop_model, torque_model, rop_results, torque_results = train_drilling_models(drilling_data)
