...
def train_drilling_models(data, rop_params=None, torque_params=None):
    ...
    if rop_params is not None:
        ...
        # Decode parameters: [hidden_neurons, dropout, lr_index, batch_size, weight_decay]
        hidden_sizes = [int(rop_params[0])]
        dropout_rate = float(np.clip(rop_params[1], 0.2, 0.5))
        
        # Decode learning rate from index
    ...
    if torque_params is not None:
        ...
        print(f"Using optimized Torque parameters:")
        print(f"  Hidden neurons: {hidden_sizes[0]}")
        print(f"  Dropout: {dropout_rate:.3f}")
        print(f"  Learning rate: {learning_rate}")
        print(f"  Batch size: {batch_size}")
