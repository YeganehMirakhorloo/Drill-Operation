...
def train_drilling_models(data):
    ...
    # Train Torque model with single hidden layer
    ...
    # AUGMENT TRAINING DATA FIRST (before validation split)
    print("\nAugmenting Torque training data...")
    X_train_aug, y_torque_train_aug = augment_training_data(
        X_train, y_torque_train,
        noise_levels=[0.05, 0.10, 0.20],
        verbose=True
    )
    ...
    X_train_split, X_val_split, y_torque_train_split, y_torque_val_split = train_test_split(
        X_train_aug, y_torque_train_aug, test_size=0.2, random_state=42
    )
